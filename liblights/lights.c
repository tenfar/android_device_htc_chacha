...
static int set_light_flashlight (struct light_device_t* dev,
		struct light_state_t const* state) {
	int err = 0;
	int on = is_lit (state);
	pthread_mutex_lock (&g_lock);
	err = write_int (FLASHLIGHT_FILE, on?255:0);
	pthread_mutex_unlock (&g_lock);

	return 0;
}

static int set_light_sharekey (struct light_device_t* dev,
		struct light_state_t const* state) {
	int err = 0;
	int on = is_lit (state);
	pthread_mutex_lock (&g_lock);
...
static int open_lights (const struct hw_module_t* module, char const* name,
	...
	else if (0 == strcmp(LIGHT_ID_SHAREKEY, name)) {
		set_light = set_light_sharekey;
	}
